...
public class IssueResolvingTimes {
	...
	public List<Pair<Integer, Double>> getIssueResolvingTimesPrediction(JiraIssue issue, List<Pair<Integer, Double>> similarityList){	
		...
		for (Assignee assignee : assigneeList)
		{
			...
			this.rtsList.get(this.rtsList.size()-1).setSimilarities(getAssigneeSimilarities(project, assignee.getAssigneeId(), similarityList));
		...
		for(ResolvingTimesStructure rts : this.rtsList){
			...
			for(Pair<Integer, Double> a : rts.getSimilarities()){
				...
				if (it == this.k || it == rts.getSimilarities().size()){
					issueResolvingTimesPrediction.add(new Pair<Integer, Double>(rts.getAssigneeID(), issueTimePrediction/alphaSum));
					break;
				}
	...
	public List<Pair<Integer, Double>> getAssigneeSimilarities(JiraProject project, int assigneeID, List<Pair<Integer, Double>> similarityList){
		@SuppressWarnings("unchecked")
		...
		for (AssignedIssue assigned : assignedIssues){ //get assignedIssues where assigned.assigneeID = assigneeID
	...
	public List<Pair<Integer, Double>> getIssuesResolvingTimes(JiraProject project, int assigneeID){
		...
		for (Pair<Integer, Double> i : rtsList.get(rtsList.size()-1).getSimilarities()){
			for (AssignedIssue assigned : assignedIssues){ //assigned = get assigned where assigned.JiraissueID = i.getkey()
				if(assigned.getJiraIssue().getJiraIssueId() == i.getKey()){
					...
					issuesResolvingTimes.add(new Pair<Integer, Double>(i.getKey(), ((finish.getTime() - start.getTime())*1.0)/(1000*60*60*24)));
				}
			}
		}
